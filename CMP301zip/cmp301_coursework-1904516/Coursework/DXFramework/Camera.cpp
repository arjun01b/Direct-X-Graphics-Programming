...
{
	//XMFLOAT3 r_up, r_position, r_lookAt;
	XMVECTOR  r_up, r_position, r_lookAt;
	float radians;

	// Setup the vector that points upwards.
	r_up = XMVectorSet(0.0f, 1.0, 0.0, 1.0f);
	// Setup the position of the camera in the world.
	// For planar reflection invert the Y position of the camera.
	r_position = XMVectorSet(position.x, -position.y + (height * 1.5f), position.z, 1.0f);

	// Calculate the rotation in radians.
	radians = rotation.y * 0.0174532925f;

	// Setup where the camera is looking.
	r_lookAt = XMVectorSet(sinf(radians) + position.x, -position.y + (height * 1.5f), cosf(radians) + position.z, 1.0f);

	// Finally create the view matrix from the three updated vectors.
	reflectionViewMatrix = XMMatrixLookAtLH(r_position, r_lookAt, r_up);
}

XMMATRIX Camera::getReflectionMatrix()
{
