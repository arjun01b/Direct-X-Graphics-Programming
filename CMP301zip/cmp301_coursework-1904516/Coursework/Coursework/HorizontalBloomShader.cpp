...
{
	initShader(L"horizontalBloom_vs.cso", L"horizontalBloom_ps.cso");
}
...
void HorizontalBloomShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, const XMMATRIX& projectionMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* depthMap, float w_screen, float close_f, float dist_f, float close_b, float dist_b,float bloomMultiplier,float brightThres)
{
