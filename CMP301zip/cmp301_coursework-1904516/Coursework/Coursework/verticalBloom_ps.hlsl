...
cbuffer BloomBuffer : register(b2)
{
...
float4 main(InputType input) : SV_TARGET
{
	...
	////float4 originalChroma;
	//originalChroma = sceneTexture.Sample(texSampler, input.tex);
	//
	//
	//float originalBrightness;
	//float resultantBrightness;
	chroma = sceneTexture.Sample(texSampler, input.tex);

	//float bloomMultiplier = 1.0f;
	float brightness;
	float texel_mea = 1.0f / h_screen;
	//float brightThres = 0.8f;
	int j = -6;
	while (j < 7)
	{
		int b = j;
		...
		float3 neighbourChroma = sceneTexture.Sample(texSampler, input.tex + float2(texel_mea * j, 0.0f));

		brightness = calcBrightness(neighbourChroma);



		if (brightness >= brightThres)
		{
			chroma.rgb += neighbourChroma * mass[b]* bloomMultiplier;
		}
