...
HShader::HShader(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
...
void HShader::initShader(const wchar_t* vsFilename, const wchar_t* psFilename)
{
...
void HShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, const XMMATRIX& projectionMatrix, ID3D11ShaderResourceView* texture, Light* light[2], ID3D11ShaderResourceView* heightmap, ID3D11ShaderResourceView* depthMap, ID3D11ShaderResourceView* depthMap2, FPCamera* camera, const XMMATRIX& lightViewMatrix, const XMMATRIX& lightProjMatrix, const XMMATRIX& spotlightViewMatrix, const XMMATRIX& spotlightProjMatrix)
{
	...
	for (int i = 0; i < 2; i++)
	{
		lightPtr->dir_ill[i] = XMFLOAT4(light[i]->getDirection().x, light[i]->getDirection().y, light[i]->getDirection().z, 1.0);
		lightPtr->chroma_diff[i] = light[i]->getDiffuseColour();
		lightPtr->chroma_amb[i] = light[i]->getAmbientColour();
		lightPtr->spec_chroma[i] = light[i]->getSpecularColour();
		lightPtr->spec_int[i].x = light[i]->getSpecularPower();
	}
	lightPtr->spec_int[1].y = cosf(60 * 0.0174533);
